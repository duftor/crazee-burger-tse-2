...
export const checkIfProductIsClicked = (idProductInMenu: string, idProductClickedOn: string) => {
...
export const getProductsToDisplay = (
	...
	const productsToDisplayed = categoryAll.isActive
		...
		: products.filter(({ categories: categoriesFromMenu }) =>
				categoriesFromMenu?.some((categoryFromMenu) => categoryFromMenu.label === activeCategory?.label)
		  )
