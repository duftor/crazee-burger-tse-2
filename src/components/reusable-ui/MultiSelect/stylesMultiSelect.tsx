...
export const stylesMultiSelect = <Option extends BaseOptions>(): StylesConfig<Option, true> => ({
	container: (base) => ({
		...base,
		width: "100%",
	}),
	// 💡 "control" : c’est le conteneur principal du composant Select. C’est là que tu peux modifier le style du champ lui-même.
	...
	multiValue: (base, state) => {
		const data = state.data as unknown as Category
		return {
			...base,
			backgroundColor: data.color ? applyOpacity(data.color, 0.1) : applyOpacity(theme.colors.purple, 0.3), // ✅ on récupère la couleur définie dans l'option
			borderRadius: 20,
	...
	option: (base, state) => {
		return {
			...
			//   : "white",
			backgroundColor: getBgColorToApply(state.data, state.isSelected, state.isFocused),
